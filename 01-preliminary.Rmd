...
save(file = here("data/eng_wg_sex_slopes_invariant.Rds"), mg2)

Mit = as_tibble(coef(mg2, simplify=T)$Male$items) %>%
  mutate(definition = rownames(coef(mg2, simplify=T)$Male$items)) %>%
  select(-g, -u) %>% 
  rename(d_m = d)
Fit = as_tibble(coef(mg2, simplify=T)$Female$items) %>%
  mutate(definition = rownames(coef(mg2, simplify=T)$Female$items)) %>%
  select(-g, -u) %>%
  rename(d_f = d)
...
thresh = median(combo$d_diff_abs) + sd(combo$d_diff_abs)
...
plot(x=fem, y=mal) # slopes are constrained to be the same...
