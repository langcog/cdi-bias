...
#table(subset(d_demo, age>22 & age<26)$sex) # 395 female, 408 male
#table(subset(d_demo, age>22 & age<26)$ses_group) # 1038 high-SES, 140 low-SES
#table(subset(d_demo, age>22 & age<26)$eth_group) # 144 nonwhite, 603 white
```
...
p1 <- sex %>% filter(age<26, age>22) %>%
  ggplot(aes(x=theta_sex, color=sex, fill=sex)) +
    geom_density(alpha=.2) + theme_classic() + xlab("Ability") + 
    geom_vline(data=sex_m, aes(xintercept=ability, color=sex)) +
    scale_color_discrete(type=sex_colors) + 
...
p2 <- ses %>%
  ggplot(aes(x=theta_ses, color=SES, fill=SES)) +
    geom_density(alpha=.2) + theme_classic() + xlab("Ability") + 
    geom_vline(data=ses_m, aes(xintercept=ability, color=SES)) + 
...
eth_m <- eth %>% group_by(`Ethnic Group`) %>%
  filter(age<26, age>22) %>%
