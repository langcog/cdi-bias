...
Full demographic data are not reported in some datasets contributed to Wordbank: sex is available for `r nrow(d_demo)-sum(is.na(d_demo$sex))` children, `r nrow(d_demo)-sum(is.na(d_demo$sex))`.
...
Our data included `r sex_tab["Female"]` female and `r sex_tab["Male"]` male participants.
