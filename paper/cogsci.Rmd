...
```{r}
# baseline Rasch model differences in ability by demographic groups
```
...
# sex
ses <- d_demo %>% filter(!is.na(sex_group), production!=0) %>%
  mutate(theta_sex = fscores(mod_intuitive_sex)) %>%
  rename(`Sex Group`=sex_group)
# cor.test(eth$production, eth$theta_eth) # .98

sex_m <- sex %>% group_by(`Sex Group`) %>%
  summarise(ability = mean(theta_sex, na.rm=T))

p1 <- sex %>%
  ggplot(aes(x=theta_sex, color=`Sex Group`, fill=`Sex Group`)) +
    geom_density(alpha=.2) + theme_classic() + xlab("Ability") + 
    geom_vline(data=sex_m, aes(xintercept=ability, color=`Sex Group`))


# SES
ses <- d_demo %>% filter(!is.na(ses_group), production!=0) %>%
  mutate(theta_ses = fscores(mod_intuitive_ses)) %>%
  rename(`SES Group`=ses_group)
# cor.test(eth$production, eth$theta_eth) # .98

ses_m <- ses %>% group_by(`Ethnic Group`) %>%
  summarise(ability = mean(theta_ses, na.rm=T))

p2 <- ses %>%
  ggplot(aes(x=theta_ses, color=`SES Group`, fill=`SES Group`)) +
    geom_density(alpha=.2) + theme_classic() + xlab("Ability") + 
    geom_vline(data=ses_m, aes(xintercept=ability, color=`SES Group`))
